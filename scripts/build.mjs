...
if(!base||!token)throw Error('Missing content export credentials. Existing published site is preserved.');
if(new URL(base).protocol!=='https:')throw Error('HTTPS export required');
async function read(params={}){const url=new URL(base);Object.entries(params).forEach(([k,v])=>url.searchParams.set(k,v));for(let i=0;i<3;i++){const r=await fetch(url,{headers:{'OAI-Sites-Authorization':'Bearer '+token},redirect:'error',signal:AbortSignal.timeout(90000)});if(r.ok)return r.json();if(i<2&&[409,429,502,503,504].includes(r.status)){await new Promise(r=>setTimeout(r,3000*(i+1)));continue;}throw Error('Content export failed: HTTP '+r.status+' (no deployment was made)');}}
